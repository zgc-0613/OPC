...
## Public Archive And University OPC UI Follow-up (2026-09-01)

This follow-up does not change AI readiness. It does not add a Provider, prompt, tool protocol, retrieval path, Agent state transition, evidence/citation rule, quota behavior, model configuration, or backend response shape. `UniversityOpcView` continues to render only the existing public preview API response and its real source URLs; pagination is a client-side visibility boundary over that already-authorized response.

The only AI-adjacent release consideration is regression protection. The final frontend, Python, repository, non-container Spring, and packaging gates pass, but the required MySQL 8.4/Testcontainers gate cannot start because the workstation has no Docker CLI/service or registered WSL distribution. Under the user's explicit instruction, the initial frontend-only atomic release completed to `/opt/opc/releases/20260901-235819`, the bilingual University navigation closure followed at `/opt/opc/releases/20260902-003155`, the mobile rail correction reached `/opt/opc/releases/20260902-081330`, and the shared navigation typography correction is current at `/opt/opc/releases/20260902-235323` with `/opt/opc/releases/20260902-081330` retained for rollback. No AI runtime, Provider, database, migration, or Assistant behavior changed. The unexecuted MySQL gate remains a documented residual risk, not a pass.
