<template>
  ...
  >
    <header class="history-sidebar-head">
      <button class="sidebar-toggle" type="button" :aria-label="collapsed ? '展开历史栏' : '收起历史栏'" @click="emit('toggle')">
        <PanelLeftOpen v-if="collapsed" :size="19" /><PanelLeftClose v-else :size="19" />
      </button>
      <strong v-if="!collapsed">研究历史</strong>
    ...
    <button class="new-research" type="button" @click="emit('new')"><Plus :size="18" /><span v-if="!collapsed">新建研究</span></button>
        ...
        <template v-for="group in groups" :key="group.label">
          ...
          >
            <input
              ...
              @blur="cancelRename"
            />
            <button v-else class="history-row-main" type="button" @click="emit('select', session)">
  ...
  <button v-if="mobileOpen" class="history-backdrop" type="button" aria-label="关闭历史抽屉" @click="emit('close-mobile')"></button>
</template>
