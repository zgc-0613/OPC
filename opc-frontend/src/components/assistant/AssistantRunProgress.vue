<template>
  <section v-if="run" class="run-progress" :class="`is-${run.status}`" :role="isError ? 'alert' : 'status'" :aria-busy="terminalSyncStatus === 'pending' ? 'true' : undefined" aria-live="polite" aria-atomic="true">
    <div class="run-main">
      ...
      <div>
        <Transition name="run-stage" mode="out-in">
          <strong :key="`${run.status}:${title}`">{{ title }}</strong>
        </Transition>
...
const controlledProgressLabels = new Set([
  '正在理解问题',
  '正在制定研究步骤',
  '正在检索案例',
  '正在检索政策',
...
const title = computed(() => {
  ...
  return status || stageLabel(props.run.currentStage, props.run)
})
const description = computed(() => {
  ...
  return errors[props.run.diagnosticCode] || ''
})
function stageLabel(stage, run) {
  if (controlledProgressLabels.has(run?.visibleProgress)) return run.visibleProgress
  if (stage === 'tool_requested' || stage === 'tool_running') {
    const tool = run?.activeToolName || run?.toolName || run?.tools?.at(-1)?.toolName
    if (tool === 'search_cases' || tool === 'compare_cases') return '正在检索案例'
    if (tool === 'search_policies') return '正在检索政策'
    if (tool === 'get_source') return '正在核验来源'
    return '正在核验来源'
  }
  return ({
    received: '正在理解问题', planning: '正在制定研究步骤', waiting_for_model: '正在理解问题',
    synthesizing: '正在整理回答',
  }[stage] || '正在理解问题')
}
</script>
...
<style scoped>
.run-progress{width:min(100%,880px);margin:6px auto 20px;padding:15px 0;border-top:1px solid #c9cdc7;border-bottom:1px solid #c9cdc7}.run-main{display:flex;align-items:flex-start;gap:11px}.run-main>div{min-width:0;flex:1}.run-main strong{font-family:'Noto Serif SC',STSong,SimSun,serif;font-size:.88rem;font-weight:600}.run-main p{margin:4px 0 0;color:#5c635c;font-size:.76rem;line-height:1.55}.run-main small{display:block;margin-top:4px;color:#7a5a31;font-size:.68rem}.run-pulse{width:9px;height:9px;margin-top:5px;border-radius:50%;background:#3d684a;}.text-command,.danger-command{display:flex;align-items:center;gap:6px;min-height:38px;padding:0 9px;border:1px solid #bfc5bd;border-radius:3px;background:#fbfbf7;color:#333933}.danger-command{border-color:#9f615b;color:#762f2a}.text-command:focus-visible,.danger-command:focus-visible{outline:2px solid rgba(74,82,74,.34);outline-offset:2px}.text-command:active{background:#e1e4dc}.danger-command:active{background:#ead5d1}.text-command:disabled,.danger-command:disabled{border-color:#d4d7d2;background:#e4e6e1;color:#8a908a}.is-failed,.is-cancelled,.is-expired,.is-evidence_insufficient{color:#6e302c}.research-plan{display:grid;grid-template-columns:88px minmax(0,1fr);gap:10px;margin:12px 0 0 20px;padding-top:10px;border-top:1px solid #d8dad5}.research-plan>span{color:#626962;font-size:.68rem;font-weight:700}.research-plan ol{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px 22px;margin:0;padding-left:20px;color:#444b44;font-size:.71rem;line-height:1.5}.research-plan li{padding-left:2px;overflow-wrap:anywhere}@media(max-width:640px),(pointer:coarse){.run-main{flex-wrap:wrap}.run-main :is(.text-command,.danger-command){min-height:44px}.research-plan{grid-template-columns:1fr;margin-left:0}.research-plan ol{grid-template-columns:1fr}}@media (hover: hover) and (pointer: fine){.text-command:hover,.danger-command:hover{border-color:#747b74;background:#eceee8}.danger-command:hover{border-color:#7a302b;background:#f2e4e1}}
.text-command,.danger-command{min-height:44px}
</style>
<style scoped>
.run-stage-enter-active{transition:opacity var(--duration-fast) var(--ease-out)}.run-stage-leave-active{transition:opacity 100ms var(--ease-out)}.run-stage-enter-from,.run-stage-leave-to{opacity:0}@media(prefers-reduced-motion:reduce){.run-stage-enter-active,.run-stage-leave-active{transition:none}}
</style>
