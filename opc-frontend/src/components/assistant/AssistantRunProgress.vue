<template>
  <section v-if="run" class="run-progress" :class="`is-${run.status}`" :role="isError ? 'alert' : 'status'">
    <div class="run-main">
      ...
      <button v-else-if="isError && canRetry" class="text-command" type="button" @click="$emit('retry')"><RefreshCw :size="14" />安全重试</button>
