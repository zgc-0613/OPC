...
const error = ref('')
const notice = ref('')
const form = ref({ rating: 'helpful', reason: 'accurate_and_useful', comment: '', revision: 0 })
...
async function load() {
  ...
  error.value = ''
  try {
    const feedback = await getResearchRunFeedback(props.run.runId)
    if (feedback) {
      form.value = {
    ...
    error.value = requestError.message || '评价暂时无法读取。'
  } finally {
    loading.value = false
  }
}
