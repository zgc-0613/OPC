<template>
  <div ref="transcript" class="conversation" data-scroll-owner="conversation" role="log" aria-live="polite" aria-relevant="additions" @scroll="handleScroll">
    <p v-if="copyFeedback" class="sr-only" :role="copyFailed ? 'alert' : 'status'" aria-live="polite">{{ copyFeedback }}</p>
    ...
    <article v-for="message in messages" :key="message.messageId" class="message" :class="`is-${message.role}`">
      ...
      <template v-else>
        <AssistantStructuredResult v-if="structuredResult(message)" :result="structuredResult(message)" :message-id="message.messageId" @citations="$emit('citations', citationMessage(message))" />
        <div v-else class="assistant-markdown" v-html="renderMessage(message)" @click="handleMarkdownClick($event, message)"></div>
      </template>
      <footer v-if="message.role === 'assistant'">
        ...
        <button v-if="messageCitations(message).length" class="citation-trigger" type="button" @click="$emit('citations', citationMessage(message), $event)"><BookOpen :size="15" />{{ messageCitations(message).length }} 条引用</button>
        <button v-if="message.runId" type="button" @click="$emit('process', message, $event)"><ListTree :size="15" />研究过程</button>
        <span>AI 生成内容，请核对原始来源</span>
      </footer>
    </article>

    <button v-if="evidenceRunId" class="evidence-summary-command" type="button" @click="$emit('evidence', $event)">
...
const emit = defineEmits(['load-older', 'prefill', 'citations', 'process', 'evidence', 'cancel', 'retry', 'resume'])
...
function handleMarkdownClick(event, message) {
  const link = event.target instanceof Element ? event.target.closest('a') : null
  if (!link || !event.currentTarget?.contains(link)) return
  const match = /^#evidence-([1-9]\d*)-([1-9]\d*)$/.exec(link.getAttribute('href') || '')
  if (!match || String(message.runId || '') !== String(props.evidenceRunId || '')) return
  const sourceId = match[2]
  const authorized = props.evidenceItems.some((item) => (
    item?.available && item?.citationId && String(item.sourceId) === sourceId
  ))
  if (!authorized) return
  event.preventDefault()
  emit('citations', citationMessage(message), Number(sourceId))
}
...
async function copyMessage(message) {
  ...
  copyTimer = window.setTimeout(() => {
    if (copiedId.value === message.messageId) copiedId.value = null
    copyFeedback.value = ''
    copyFailed.value = false
  }, 2400)
}
