...
- Deployment preflight stops with `OPC_SSH_PASSWORD is not set; deployment stopped before SSH`. The password will not be embedded in a command, file, log, or report, so production remains unchanged and the real DeepSeek probe has not run.
...
- Production deployment remains pending because the secure deployment environment variables are absent; no production state was changed during this pass.
